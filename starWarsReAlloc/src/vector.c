...
/*** TO BE DONE START ***/
    ...
    free((void*)my_v->e_array); //Libero memoria allocata per l'array interno

/*** TO BE DONE END ***/

    free((void*)my_v); //Libero memoria allocata per la struct generale
}
...
void* v_at(vector_type v, iterator i){
    ...
    if ( i == iterator_end )
    	i = v->no_e -1; //Se l'iterator coincide con iterator_end gli do il numero dell'ultimo elemento
    if ( i < 0 || i >= v->no_e )
    	fail("v_at() Error: no such element in this vector");

/*** TO BE DONE START ***/

    return (v->e_array)+(i*v->e_sz); //Ritorno indirizzo della cella che sto cercando
...
void v_push_back(vector_type v, void* new_val){
    if ( v->no_e >= v->cur_cap ) {
    	...
    	v->cur_cap += (v->cur_cap)? v->cur_cap : 2; //Aumento capacity(raddoppio)

/*** TO BE DONE START ***/

      v->e_array = realloc(v->e_array, ((v->cur_cap)*(v->e_sz))); //Alloco un "blocchetto" di capacity in più
...
/*** TO BE DONE START ***/
      ...
      memcpy ((v->e_array)+(v->no_e*v->e_sz), new_val, v->e_sz); //memcpy(destination, source, num bytes), copio v->e_sz byte da new_val
                                                                 //alla posizione data come dest, (v->e_array...), ovvero l'ultima del vector

/*** TO BE DONE END ***/

    (v->no_e)++; //Aumento numero di elementi
}
...
/*** TO BE DONE START ***/
    ...
    v->e_array = (void*) realloc(v->e_array, ((v->cur_cap)*(v->e_sz))); 
...
/*** TO BE DONE START ***/
	 ...
	 memcpy (v_at(v, moved+to_insert),  v_at(v, moved), v->e_sz); //Sposto da (v,moved) a (v, moved+to_insert)
																					  //Moved è il punto di partenza, Moved+to_insert è l'arrivo
																					  //Sposto tutti i valori di 1 per inserire in n
...
/*** TO BE DONE START ***/
	 ...
	 memcpy (v_at(v, start_i+to_insert), new_val, v->e_sz); //Copio il valore dall'indirizzo new_val all'indirizzo della cella giusta
...
/*** TO BE DONE START ***/
	 ...
	 for( i ; i < j ; i++) v_pop_back(v); //Ogni chiamata di pop_back elimina l'ultimo elemento, lo chiamo finchè i continua ad essere minore di j
